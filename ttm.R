...
# Ensure the england-latest.osm.pbf file is in the correct directory
osm_file <- 'data/england-latest.osm.pbf'
if (!file.exists(paste0(r5r_dir, '/', basename(osm_file)))) {
  file.copy(osm_file, paste0(r5r_dir, '/', basename(osm_file)), overwrite = TRUE)
}

# Copy GTFS files to create R5 network
gtfs_files <- list.files('data', recursive = TRUE, pattern = 'gtfs\\.zip$', full.names = TRUE)
lapply(gtfs_files, function(x) file.copy(x, paste0(r5r_dir, '/', basename(x)), overwrite = TRUE))
